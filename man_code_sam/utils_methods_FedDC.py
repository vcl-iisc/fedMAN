...
def train_FedDC(data_obj, act_prob,n_minibatch,
                  ...
                  suffix = '', trial=True, rand_seed=0, lr_decay_per_round=1,args = None):
        ...
        for i in range(saved_itr+1, com_amount):
            ...
            for clnt in selected_clnts:
                ...
                if args.add_reg == 0:
                    ...
                    clnt_models[clnt] = train_model_FedDC_reg(model, model_func, alpha,local_update_last, global_update_last,global_mdl, hist_i, 
                                                     trn_x, trn_y, learning_rate * (lr_decay_per_round ** i), 
                                                     batch_size, epoch, print_per, weight_decay, data_obj.dataset, sch_step, sch_gamma,args)
            ...
            if (not trial) and ((i+1) % save_period == 0):
                torch.save(avg_model_sel.state_dict(), 'Output/%s/%s/ins_avg_%dcom.pt' 
                           %(data_obj.name, suffix, (i+1)))
                torch.save(all_model.state_dict(), 'Output/%s/%s/all_avg_%dcom.pt' 
                           %(data_obj.name, suffix, (i+1)))
                torch.save(cur_cld_model.state_dict(), 'Output/%s/%s/cld_avg_%dcom.pt' 
                           %(data_obj.name, suffix, (i+1)))

                np.save('Output/%s/%s/%d_com_trn_sel_clt_perf.npy' %(data_obj.name, suffix, (i+1)), trn_sel_clt_perf[:i+1])
                np.save('Output/%s/%s/%d_com_tst_sel_clt_perf.npy' %(data_obj.name, suffix, (i+1)), tst_sel_clt_perf[:i+1])
                np.save('Output/%s/%s/%d_com_trn_all_clt_perf.npy' %(data_obj.name, suffix, (i+1)), trn_all_clt_perf[:i+1])
                np.save('Output/%s/%s/%d_com_tst_all_clt_perf.npy' %(data_obj.name, suffix, (i+1)), tst_all_clt_perf[:i+1])

                np.save('Output/%s/%s/%d_com_trn_cur_cld_perf.npy' %(data_obj.name, suffix, (i+1)), trn_cur_cld_perf[:i+1])
                np.save('Output/%s/%s/%d_com_tst_cur_cld_perf.npy' %(data_obj.name, suffix, (i+1)), tst_cur_cld_perf[:i+1])
                    
                # save parameter_drifts

                np.save('Output/%s/%s/%d_hist_params_diffs.npy' %(data_obj.name, suffix, (i+1)), parameter_drifts)
                np.save('Output/%s/%s/%d_clnt_params_list.npy' %(data_obj.name, suffix, (i+1)), clnt_params_list)
                     

                if (i+1) > save_period:
                    # Delete the previous saved arrays
                    os.remove('Output/%s/%s/%d_com_trn_sel_clt_perf.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_com_tst_sel_clt_perf.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_com_trn_all_clt_perf.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_com_tst_all_clt_perf.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_com_trn_cur_cld_perf.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_com_tst_cur_cld_perf.npy' %(data_obj.name, suffix, i+1-save_period))

                    os.remove('Output/%s/%s/%d_hist_params_diffs.npy' %(data_obj.name, suffix, i+1-save_period))
                    os.remove('Output/%s/%s/%d_clnt_params_list.npy' %(data_obj.name, suffix, i+1-save_period))
