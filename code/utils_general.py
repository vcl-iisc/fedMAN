...
def train_feddyn_mdl_reg(model, model_func, alpha_coef, avg_mdl_param, local_grad_vector, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, 
...
def compute_last_layer_loss(act_in,label):
    #print("act_in:",act_in)
    #print("label:",label)
    mask = torch.ones(act_in.shape).to(device)
    mask.scatter_(1, label.long(), 0.)
    #print("mask:",mask[:3])
    return torch.mean(mask*(act_in**2))
    

def train_fedavgreg_mdl(model, avg_model_param_, mu, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name,args):
    n_trn = trn_x.shape[0]
    trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
...
def train_model_FedDC_reg(model, model_func, alpha, local_update_last, global_update_last, global_model_param, hist_i, trn_x, trn_y, 
                    learning_rate, batch_size, epoch, print_per,
...
def train_model_speed(args,model, model_func, alpha_coef, avg_mdl_param, hist_params_diff, train_x, train_y, 
                    learning_rate, batch_size, epoch, print_per,
                    weight_decay, dataset_name, sch_step, sch_gamma, rho, print_verbose=False):
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_train/batch_size))):
            ...
            def defined_backward(loss):
                loss.backward()
            paras = [batch_x, batch_y, loss_fn, model, defined_backward,args.mu_mean]
        ...
        if print_verbose and (e+1) % print_per == 0:
            loss_train, acc_train = get_acc_loss(train_x, train_y, model, dataset_name, weight_decay)
