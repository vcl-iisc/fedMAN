...
class DatasetObject:
    ...
    def set_data(self):
        ...
        if not os.path.exists('%s/%s' %(self.data_path, self.name)):
            ...
            if self.dataset == 'CIFAR10C' or self.dataset == 'CIFAR100C':

                if self.dataset == 'CIFAR10C':
                    ds_pth_name = 'Data/CIFAR-10-C'
                    self.n_cls = 10
                else:    
                    ds_pth_name = 'Data/CIFAR-100-C'
                    self.n_cls = 100


                cifar_corrupt_trn =  np.load(ds_pth_name +'/Train/impulse_noise.npy')
                cifar_corrupt_trn_labels = np.load(ds_pth_name + '/Train/labels.npy')

                cifar_corrupt_tst =  np.load(ds_pth_name + '/Test/impulse_noise.npy')
                cifar_corrupt_tst_labels =  np.load(ds_pth_name + '/Test/labels.npy')

                cifar_train = np.load(ds_pth_name + '/Train/clean_train_images.npy')
                cifar_train_labels = np.load(ds_pth_name +'/Train/clean_train_labels.npy')

                cifar_test = np.load( ds_pth_name + '/Test/clean_test_images.npy')
                cifar_test_labels = np.load(ds_pth_name + '/Test/clean_test_labels.npy')
...
#               
            ...
            if self.dataset == 'CIFAR100' or  self.dataset == 'CIFAR100C' :
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_x, data_y=True, train=False, dataset_name=''):
        ...
        elif self.name == 'CIFAR10' or self.name == 'CIFAR100' or self.name == 'CIFAR10C' or  self.name == 'CIFAR100C':
            self.train = train
